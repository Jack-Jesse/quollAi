...
// ─── Liquid Button (with Framer Motion shimmer) ─────────────────────────────
  ...
  return (
    <>
      <motion.button
        ...
        whileHover={{ scale: 1.05, transition: { type: "spring" as const, stiffness: 400, damping: 25 } }}
        whileTap={{ scale: 0.97 }}
        {...props}
      >
        ...
        {/* Framer Motion shimmer gleam that sweeps across on hover */}
        <motion.div
          className="absolute inset-0 z-0 rounded-md bg-gradient-to-r from-transparent via-white/25 to-transparent pointer-events-none"
...
// ─── Metal Button (with Framer Motion) ─────────────────────────────────────
...
const metalButtonStyles = (
  ...
  return {
    wrapper: cn(
      "relative inline-flex rounded-md p-[1.25px]",
      colors.outer,
    ),
    inner: cn(
...
function MetalButton({ children, className, variant = "default", onDrag: _onDrag, onDragStart: _onDragStart, onDragEnd: _onDragEnd, onAnimationStart: _onAnimationStart, ...props }: MetalButtonProps) {
  const [isTouchDevice, setIsTouchDevice] = React.useState(false);
  const styles = metalButtonStyles(variant);
  ...
  return (
    ...
    >
      <motion.div
        ...
        whileHover={!isTouchDevice ? { filter: "brightness(1.05)" } : undefined}
      ...
      <motion.button
        ...
        {...props}
      >
        {/* Framer Motion shine sweep on tap */}
        <motion.div
        ...
        <motion.div
          ...
          initial={{ opacity: 0 }}
          whileHover={!isTouchDevice ? { opacity: 1 } : undefined}
